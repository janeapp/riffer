...
Riffer publishes a completion event for every operation — each LLM call, agent run, tool call, and guardrail — through a synchronous in-process bus under `config.events`. Subscribers receive every event and filter by type; observability backends (metrics, Datadog, logs) are subscribers you write, so riffer ships no metrics backend of its own. See [Events](17_EVENTS.md) for the event schema and example subscribers.
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `subscribe(callable)` | Registers a subscriber — an object responding to `#call(event)`, or a block. Returns it for a later `unsubscribe`. A `nil` subscriber is ignored, so a backend builder that returns `nil` when its SDK is absent stays a one-liner. Raises `Riffer::ArgumentError` for a non-`nil`, non-callable value. |
