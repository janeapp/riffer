...
Riffer publishes a completion event for every operation — each LLM call, agent run, tool call, and guardrail — through a synchronous in-process bus under `config.events`. A subscriber takes every event and filters by type, or subscribes to a single event class; observability backends (metrics, Datadog, logs) are subscribers you write, so riffer ships no metrics backend of its own. See [Events](17_EVENTS.md) for the event schema and example subscribers.

```ruby
Riffer.configure do |config|
  # Any object responding to #call(event), or a block:
  config.events.subscribe { |event| MyBackend.record(event) }
end
```

| Method / option               | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `subscribe([type], callable)` | Registers a subscriber — an object responding to `#call(event)`, or a block. Pass an event class as an optional first argument to receive only that type (and its subclasses); omit it to receive every event. Returns the subscriber for a later `unsubscribe`. A `nil` subscriber is ignored, so a backend builder that returns `nil` when its SDK is absent stays a one-liner. Raises `Riffer::ArgumentError` for a non-`nil`, non-callable value or a type that isn't a `Riffer::Events::Base` subclass. |
