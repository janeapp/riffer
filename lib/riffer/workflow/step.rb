...
# Riffer::Workflow::Step represents a single step in a workflow, 
# which can be either an AgentStep or a custom step that implements the 
# execute method.
# Each step can define its own input and output schemas, which are used to 
# validate the data
class Riffer::Workflow::Step
    # Defines or returns the step's input schema.
    #
    ...
    # Defines or returns the step's output schema.
    #
    ...
    # Validates the input against the input schema and the execute output 
    ...
    # Executes the step with the given input. This method should be 
    # overridden by subclasses to implement the actual logic of the step.
    #--
    #: (Hash[Symbol, untyped]) -> Hash[Symbol, untyped]
    def execute(input)
