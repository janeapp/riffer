...
class Riffer::Agent
  ...
  # and another runs it. Model and instructions Procs are resolved
  # against +context+ at call time, so the result is a frozen snapshot —
  # the receiver gets the values produced for the given context, not the
  # procs themselves.
  #
  # Slots intentionally omitted from the hash:
  #
  ...
  # * +uses_tools+, +tool_runtime+, +guardrails+ — these slots reference
  #   user-defined classes whose portable representation requires
  #   resolving constants by name on the receiver. That's deferred to a
  #   future iteration; the receiver should configure these directly on
  #   its own subclass for now.
  #
  #--
  ...
  #
  # The receiver is responsible for configuring +instructions+, +skills+,
  # +uses_tools+, +tool_runtime+, and +guardrails+ on its own — the
  # config carries only primitive runtime settings. See #to_config for
  # the rationale.
  #
  ...
  #
  #--
  #: (Hash[untyped, untyped]) -> singleton(Riffer::Agent)
  def self.from_config(config)
    config = Riffer::Helpers::HashNormalizer.deep_symbolize_keys(config)
