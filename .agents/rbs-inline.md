...
- `rbs_collection.yaml` / `rbs_collection.lock.yaml` — external gem types come from `rbs collection`: gem-shipped sigs (`openai`, `anthropic`, `aws-sdk-*`) plus [gem_rbs_collection](https://github.com/ruby/gem_rbs_collection) entries (`async`, stdlib like `yaml`, `cgi-escape`). Steep loads the lockfile automatically. Run `bundle exec rbs collection install` after `bundle install` (`bin/setup` does both); the install dir `.gem_rbs_collection/` is git-ignored. riffer itself is `ignore: true` in the config — the `Steepfile` already loads its sig dirs directly.
- `sig/_private/` — signatures that must **not** ship. RBS **skips** `_`-prefixed directories in library mode, so consumers never load them; riffer's own `steep check` does (via the `Steepfile`). Two kinds, by predictable path: external-gem signatures are named by gem at the top level (`mcp.rbs`, `zeitwerk.rbs`, `openai.rbs`, `anthropic.rbs`, `aws-sdk-core/*` — full stubs for gems the collection has no types for, plus arity patches for the provider SDKs); riffer's own hidden stubs mirror `lib/` under `riffer/` (e.g. `riffer/providers/anthropic.rbs` declares the SDK-typed `@client` ivar).
