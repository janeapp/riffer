...
describe Riffer::Messages::User::FilePart do
  describe ".new" do
    ...
    it "creates a file part with url and media_type" do
      file = Riffer::Messages::User::FilePart.new(url: "https://example.com/image.png", media_type: "image/png")
    ...
    it "accepts optional filename" do
      file = Riffer::Messages::User::FilePart.new(data: "aGVsbG8=", media_type: "image/png", filename: "photo.png")
    ...
    it "raises when neither data nor url is provided" do
      error = expect do
        Riffer::Messages::User::FilePart.new(media_type: "image/png")
    ...
    it "raises for unsupported media type" do
      error = expect do
        Riffer::Messages::User::FilePart.new(data: "aGVsbG8=", media_type: "video/mp4")
      end.must_raise(Riffer::ArgumentError)
    ...
    it "accepts a valid sha256 and downcases it" do
      ...
      file = Riffer::Messages::User::FilePart.new(data: "aGVsbG8=", media_type: "image/png", sha256: sha256)
    ...
    it "raises for a malformed sha256 string" do
      error = expect do
        Riffer::Messages::User::FilePart.new(data: "aGVsbG8=", media_type: "image/png", sha256: "not-a-hash")
      end.must_raise(Riffer::ArgumentError)
    ...
    it "raises for a non-String sha256 instead of crashing on #match?" do
      error = expect do
        Riffer::Messages::User::FilePart.new(data: "aGVsbG8=", media_type: "image/png", sha256: 123)
      end.must_raise(Riffer::ArgumentError)
      expect(error.message).must_match(/Invalid sha256/)
    end
  end
  ...
  describe ".from_url" do
    it "stores the url and detects media type from extension" do
      file = Riffer::Messages::User::FilePart.from_url("https://example.com/photo.jpg")
    ...
    it "accepts explicit media_type" do
      file = Riffer::Messages::User::FilePart.from_url("https://example.com/file", media_type: "application/pdf")
    ...
    it "raises when media type cannot be detected" do
      error = expect do
        Riffer::Messages::User::FilePart.from_url("https://example.com/file")
      end.must_raise(Riffer::ArgumentError)
  ...
  describe ".from_hash" do
    it "passes through FilePart objects" do
      file = Riffer::Messages::User::FilePart.new(data: "aGVsbG8=", media_type: "image/png")
      result = Riffer::Messages::User::FilePart.from_hash(file)

      expect(result).must_equal file
    end

    it "converts url hash" do
      result = Riffer::Messages::User::FilePart.from_hash({ url: "https://example.com/photo.jpg",
                                                            media_type: "image/jpeg", })

      expect(result).must_be_instance_of Riffer::Messages::User::FilePart
      expect(result.url).must_equal "https://example.com/photo.jpg"
    end

    it "forwards filename for a url hash" do
      result = Riffer::Messages::User::FilePart.from_hash(
    ...
    it "converts data hash" do
      result = Riffer::Messages::User::FilePart.from_hash({ data: "aGVsbG8=", media_type: "image/png" })
    ...
    it "raises for invalid hash" do
      error = expect do
        Riffer::Messages::User::FilePart.from_hash({ media_type: "image/png" })
      end.must_raise(Riffer::ArgumentError)
    ...
    it "raises for a non-String sha256 in a data hash instead of crashing on #match?" do
      error = expect do
        Riffer::Messages::User::FilePart.from_hash({ data: "aGVsbG8=", media_type: "image/png", sha256: 123 })
      end.must_raise(Riffer::ArgumentError)
      expect(error.message).must_match(/Invalid sha256/)
    end
    ...
    it "raises for a non-String sha256 in a url hash instead of crashing on #match?" do
      error = expect do
        Riffer::Messages::User::FilePart.from_hash({ url: "https://example.com/file.pdf", media_type: "application/pdf",
                                                     sha256: 123, })
      end.must_raise(Riffer::ArgumentError)
      expect(error.message).must_match(/Invalid sha256/)
    end
  ...
  describe "#image?" do
    it "returns true for image media types" do
      file = Riffer::Messages::User::FilePart.new(data: "aGVsbG8=", media_type: "image/jpeg")

      expect(file.image?).must_equal true
    end

    it "returns false for document media types" do
      file = Riffer::Messages::User::FilePart.new(data: "aGVsbG8=", media_type: "application/pdf")
  ...
  describe "#data_bytes" do
    it "decodes the base64 data" do
      file = Riffer::Messages::User::FilePart.new(data: Base64.strict_encode64("hello"), media_type: "text/plain")
    ...
    it "decodes line-wrapped Base64.encode64 output" do
      file = Riffer::Messages::User::FilePart.new(data: Base64.encode64("hello" * 20), media_type: "text/plain")
    ...
    it "raises Riffer::FileEncodingError for data that isn't base64 at all" do
      file = Riffer::Messages::User::FilePart.new(data: "not base64!!!", media_type: "text/plain")
  ...
  describe "MEDIA_TYPES" do
    it "includes jpeg extensions" do
      expect(Riffer::Messages::User::FilePart::MEDIA_TYPES[".jpg"]).must_equal "image/jpeg"
      expect(Riffer::Messages::User::FilePart::MEDIA_TYPES[".jpeg"]).must_equal "image/jpeg"
    end

    it "includes png" do
      expect(Riffer::Messages::User::FilePart::MEDIA_TYPES[".png"]).must_equal "image/png"
    end

    it "includes pdf" do
      expect(Riffer::Messages::User::FilePart::MEDIA_TYPES[".pdf"]).must_equal "application/pdf"
    end
  end
end
