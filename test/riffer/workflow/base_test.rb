...
describe Riffer::Workflow::Base do
    it "input_schema throws an ArgumentError if input schema is invalid" do
        error = expect {
    ...
    it "input_schema returns an input schema defined by a block" do
    ...
    it "input_schema returns an input schema defined by a Riffer::Params object" do
    ...
    it "output_schema throws an ArgumentError if output schema is invalid" do
        error = expect {
            Class.new(Riffer::Workflow::Base) do
    ...
    it "output_schema returns an output schema defined by a block" do
        klass = Class.new(Riffer::Workflow::Base) do
            output_schema do
    ...
    it "output_schema returns an output schema defined by a Riffer::Params object" do
        params = Riffer::Params.new do
    ...
    it "step validates that the step_class is a subclass of Riffer::Workflow::Step" do
        error = expect {
            Class.new(Riffer::Workflow::Base) do
    ...
    it "step validates that the workflow has an input schema defined before adding steps" do
        error = expect {
    ...
    it "step validates that the workflow has an output schema defined before adding steps" do
        error = expect {
            step1 = Class.new(Riffer::Workflow::Step) do
                input_schema do
                    required :start_num, Integer
    ...
    it "step validates that the step has an input schema defined" do
        error = expect {
            step1 = Class.new(Riffer::Workflow::Step) do
                output_schema do
                    required :result, Integer
    ...
    it "step validates that the step has an output schema defined" do
        error = expect {
            step1 = Class.new(Riffer::Workflow::Step) do
                input_schema do
                    required :start_num, Integer
    ...
    it "step validates that the first step's input schema matches the workflow's input schema" do
        error = expect {
            step1 = Class.new(Riffer::Workflow::Step) do
                input_schema do
                    required :age, Integer
    ...
    it "step validates that each step's output schema matches the next step's input schema" do
        error = expect {
            step1 = Class.new(Riffer::Workflow::Step) do
                input_schema do
                    required :name, String
    ...
    it "step adds the step to the workflow's steps" do
        step1 = Class.new(Riffer::Workflow::Step) do
            input_schema do
                required :start_num, Integer
            end
    ...
    it "execute without steps throws a configuration error" do
        error = expect {
            Class.new(Riffer::Workflow::Base) do
                input_schema do
                    required :start_num, Integer
    ...
    it "execute validates that the last step's output schema matches the workflow's output schema" do
        step1 = Class.new(Riffer::Workflow::Step) do
            input_schema do
                required :start_num, Integer
            end
    ...
    it "execute with mismatched input schema throws a schema validation error" do
        step1 = Class.new(Riffer::Workflow::Step) do
            input_schema do
                required :start_num, Integer
            end
    ...
    it "execute with step that raises error returns a workflow result with failed status and error message" do
        step1 = Class.new(Riffer::Workflow::Step) do
            input_schema do
                required :start_num, Integer
            end
    ...
    it "execute with successful steps returns a workflow result with succeeded status and final output" do
        step1 = Class.new(Riffer::Workflow::Step) do
            input_schema do
                required :start_num, Integer
            end
