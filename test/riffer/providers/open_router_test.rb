...
describe Riffer::Providers::OpenRouter do
  ...
  describe "#build_request_params" do
    ...
    it "forwards arbitrary options to the request body" do
      params = provider.send(:build_request_params, [user_message], "openai/gpt-4o-mini", {temperature: 0.5, max_tokens: 100})
    ...
    it "passes hash reasoning through verbatim" do
      params = provider.send(:build_request_params, [user_message], "deepseek/deepseek-r1", {reasoning: {effort: "medium", max_tokens: 5000}})
    ...
    it "passes OpenRouter `provider` option through verbatim" do
      ...
      params = provider.send(:build_request_params, [user_message], "anthropic/claude-sonnet-4.6", {provider: provider_block})
    ...
    it "passes `transforms` array through verbatim" do
      params = provider.send(:build_request_params, [user_message], "openai/gpt-4o-mini", {transforms: ["middle-out"]})
  ...
  describe "tool conversion" do
    ...
    it "wraps the tool definition in a function envelope" do
      params = provider.send(:build_request_params, [Riffer::Messages::User.new("hi")], "openai/gpt-4o-mini", {tools: [weather_tool]})
      ...
      it "returns Assistant message with content" do
        VCR.use_cassette("Riffer_Providers_OpenRouter/tool_calling/_generate_text/with_tool_message") do
          ...
          messages = [
            ...
            Riffer::Messages::Tool.new("The weather in Toronto is 15 degrees Celsius.", tool_call_id: "call_abc", name: "get_weather")
          ]
          result = provider.generate_text(
            messages: messages,
            model: "anthropic/claude-haiku-4.5",
  ...
  describe "#stream_text tool-call edge cases" do
    ...
    it "assigns distinct fallback ids when multiple tool calls arrive without ids" do
      ...
      expect(item_ids).must_equal ["tool_0", "tool_1"]
      expect(call_ids).must_equal ["tool_0", "tool_1"]
    end
  end
