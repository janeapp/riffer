...
describe Riffer::Agent::Session::Repair do
  ...
  describe ".fill_orphans" do
    ...
    describe "when experimental_history_healing is off" do
      ...
      it "returns the same array reference" do
        new_messages, = Riffer::Agent::Session::Repair.fill_orphans(messages)

        expect(new_messages).must_be_same_as messages
      end

      it "returns an empty filled array" do
        _, filled = Riffer::Agent::Session::Repair.fill_orphans(messages)
    ...
    describe "when experimental_history_healing is on" do
      ...
      it "returns input unchanged when there are no orphans" do
        ...
        new_messages, filled = Riffer::Agent::Session::Repair.fill_orphans(messages)
      ...
      it "returns every filled call_id in order" do
        ...
        expect(filled).must_equal %w[c_a c_b]
      end
      ...
      it "does not mutate the input array" do
        ...
        Riffer::Agent::Session::Repair.fill_orphans(messages)
  ...
  describe ".prune_orphans" do
    ...
    describe "when experimental_history_healing is off" do
      it "returns the same array reference" do
        ...
        expect(Riffer::Agent::Session::Repair.prune_orphans(messages)).must_be_same_as messages
      end
    end
    ...
    describe "when experimental_history_healing is on" do
      ...
      it "does not mutate the input array" do
        ...
        Riffer::Agent::Session::Repair.prune_orphans(messages)
